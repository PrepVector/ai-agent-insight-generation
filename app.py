...
import pandas as pd
from ba_agent import format_questions_to_text, parse_text_to_questions, run_business_analysis
...
def main():
    ...
    metadata_file = st.sidebar.file_uploader("Upload your metadata (TXT file)", type=['txt'])
        ...
        if st.button("Generate EDA Questions") or st.session_state.generated_questions:
            ...
            if st.button("Run EDA Analysis"):
                with st.spinner("Running EDA analysis..."):
                    ...
                    results = run_eda_analysis(updated_questions)
